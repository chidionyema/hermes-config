...
| Wrapper script that auto-writes a fixed report path clobbers richer human-written content (learned 2026-08-16) | Recovery-from-silent-stretch pattern: agent writes a full audit analysis to `~/.hermes/reports/<name>-YYYY-MM-DD.md`, then runs a wrapper script that captures a probe dump to the SAME path → the rich analysis is overwritten with a thin dump, the audit deliverables disappear | When a wrapper script is the cron-target for a `no_agent` job, the wrapper must (a) append its own section under a `## Auto-executed fixes` heading rather than overwrite the file, OR (b) write to a separate path (e.g., `*-probe.md`) and let the agent merge. The recovery-run flow for cron silent-stretch is: agent writes its own analysis first, THEN installs the wrapper for tomorrow — never the other way around. A wrapper that overwrites the agent's hand-written deliverable is itself a dropped ball. |
...
| Forward a subagent's "shipped" receipts without re-verifying on disk | Chidi: *"You're testing whether I stay vigilant on receipts. Good."* — the subagent's claim that X is fixed does not make X fixed; the system that catches subagent lies is the same one that catches Otto's lies (disk state) | **Always re-probe before forwarding.** A subagent report is a self-report, not a verified fact. For any non-trivial claim (commit exists, file changed, test passes, fix shipped): run the probe yourself (`git log/show`, `grep`, `cat`, `pytest -q`), then forward. The probe cost is sub-second; the trust loss when you forward a lie is multi-turn. Exception: when the subagent returned a verifiable handle AND you already verified it (URL fetched, file stat'd, content read back). The tell that you skipped this: your forwarded message contains words like "Claude shipped" or "the commit is" without a `file:line`, `git show <sha>:` snippet, or `pytest N passed` line attached. |
| Sit on receipts after a fix is "shipped" — wait for the user to direct next work | Chidi: *"You are supposed to be working on this"* / *"I don't see any evidence of it running"* — a fix that lands but is followed by silence is a half-ship; the user is now auditing whether other things you declared done are also un-evidenced | When a fix lands and the user's next message implies they expected more, the next move is **probe the related surface** — adjacent cron jobs, sibling entries, the broader pipeline the fix touched — and surface what you find. Sitting on the receipt while the user has to re-prompt is the same dropped-ball shape as "standing by." The receipt is the start, not the end. |
| Trust a "polish" / cleanup commit message at face value — never look at the actual diff | The 2026-08-04 `/summary` bug "came back" because a polish commit removed the bypass while claiming em-dash cleanup; the only way to find this was `git show <commit>` and reading the actual `-`/`+` lines | When a commit message claims unrelated cleanup, **read the diff before merging.** If the `-` lines don't match the message ("replace em-dashes" should remove em-dashes, not bypass code), reject or investigate. The fix was a module-load invariant test (`assert "summary" in ACTIVE_SESSION_BYPASS_COMMANDS`) that pins the bypass entry — see `references/slash-command-mid-turn-bypass-recipe.md` for the full pattern. |
| Topic-drift: the user says "talking about X" and the agent continues on Y | The user corrected the topic and the agent appended unsolicited Y content before addressing X. | When the user says "I'm talking about X" or similar, the response must start with X and drop Y entirely. Any residual Y content is a dropped ball. The topic correction is a directive, not a suggestion. |
| Default to delegating-to-Claude when the agent has the tools to do the work itself | Chidi: *"Why dispatch to Claude when you are Claude, you can do the work yourself, just follow good engineering practices"* — Otto treats Claude as the default executor for in-scope work it can do with `terminal` + `patch` + `write_file` + `git`. The delegate_task tool exists for offloading context-flooding subtasks, not as a reflex. | **Otto has the gateway lane and the same substrate; the dispatch-tax includes (a) the lost thread context Claude would need to be re-handed, (b) the user's "why are you relaying this?" friction, (c) Claude's iteration cap (50 calls) that often burns through investigation without reaching the write phase.** Decision rule before `delegate_task`: (1) Can I do this in <25 tool calls? — do it. (2) Does the work require reading >5 large files that would flood my context? — delegate, but provide the punch list, not the question. (3) Is the work in `gateway/` AND the user has explicitly granted a lane override? — do it, committing with `HERMES_LANE=claude` and verifying the test gate passes before declaring success. (4) Am I staring at a Claude session that already exhausted its budget? — kill it, take the punch list it produced, and finish the write myself. The wrong default is "delegate and hope." |
| Re-state a plan as the answer to "is X done?" | The user asks a status question and the agent re-states the numbered plan from the previous turn instead of returning receipts or starting execution. A plan is a *promise of execution*, not execution. | When asked a status question ("is X done?", "did you finish?", "where are we?"), the answer is always one of: (a) receipts showing X is done, (b) an honest "not yet, here it is now" followed by the execution, or (c) a specific blocker with the exact next step. Never re-state a prior plan as the answer. If the prior turn ended in a plan and the user didn't say "go," the next turn is the execution, not a re-pitch. The receipt IS the user-facing summary — plans are scaffolding, not deliverables. |
...
| Respond to a single-word user message ("Otto?", "B", "then?") with "Yeah, I'm here" / "On it" without acting | Chidi sent a one-word follow-up after Otto's last message declared work complete — Otto treated it as a check-in and answered conversationally instead of treating it as the signal it actually was: "you went quiet / I'm waiting / prove you're still working." A user prompt with no task in it is a probe, not a greeting. | When the user's last message is short, non-task-bearing, and follows a "fix is shipped" receipt from Otto: assume Otto's prior turn sat on the receipt instead of probing the related surface. The next move is to probe the adjacent surface (sibling crons, sibling entries, the broader pipeline the prior fix touched) and surface what you find BEFORE answering conversationally. If you genuinely have nothing left to do, the answer is one line: "fix X shipped at <sha>, nothing else open from this thread" — not a greeting. The single-word follow-up is the user being patient one more time before they lose it. |
...
- **Slash-command mid-turn bypass recipe** — `references/slash-command-mid-turn-bypass-recipe.md` — the exact 4-step pattern for fixing a registered slash command that the running-agent catch-all rejects with "can't run mid-turn" (add explicit branch + add to `ACTIVE_SESSION_BYPASS_COMMANDS` frozenset + add a test for both + add a module-load invariant test against future polish commits). Loaded when any registered `/command` is rejected while an agent is running.
...
- **Externally-injected broken-policy pattern** — `references/externally-injected-broken-policy-pattern.md` — discovered 2026-08-16: the 3-gate skeleton-dedup defense only catches policies created by the near-miss auto-creation path, NOT policies injected by subagent/strategist/otto-learn that share identical rule text. Symptom: `~/.hermes/logs/maintenance/<date>.md` shows 25+ similarity-1.00 pairs with different ids. Fix recipe: archive the whole family + patch the consolidation pass (NOT the auto-creation gate).
