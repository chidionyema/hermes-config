...
|---|---|---|---|---|---|
| **F-NEW-13** | Cron timeouts (`daily-strategist-audit` 1030s/600s, `morning-briefing` 937s/600s). Jobs being fast-forwarded. | Ops | 24h | Either raise `cron.idle_timeout` from 600s OR disable the affected jobs. Verify `morning-briefing` runs to completion at next 09:00 BST. | config change; revert by setting old timeout |
| **F-NEW-14** | Telegram sustained DNS/connectivity failure (2,071 warnings; both primary DNS and fallback IPs failing) | Ops | 24h | `dig api.telegram.org` + `dig @8.8.8.8 api.telegram.org` from affected host. Identify local DNS vs routing issue. Apply fix. | depends on root cause |
| **F-NEW-2-E** | `test_panel_fail_closed_without_coordinator` fails — production `view.ok=True` when coordinator unreachable. Security/correctness bug. | Eng | 48h | `estate.py:render_panel_view()` returns `ok=False` when `_load_coordinator()` raises. Add the test case to the run-before-deploy gate. | ~60 min |
| **F-NEW-2-A** | SPINE ordering: 7 tests expect `[refresh, run, tune, find]` but production emits `[refresh, run, sdlc, find]` | Eng | 48h | Either change `panel_chrome.py:nav()` to emit `estate:tune` at position 2, OR update tests if new order is intentional. | ~30 min |

### P2 — Medium (next sprint)

| ID | Finding | Owner | Due | Definition of done | Effort |
|---|---|---|---|---|---|
| **F-NEW-2-B** | Home card duplicates SDLC (3 tests) | Eng | 1 wk | Remove duplicate `estate:sdlc` from `mission.py:mission_buttons()`. | ~15 min |
| **F-NEW-2-C** | `test_find_panel_does_not_offer_itself` — test doesn't account for SPINE | Eng | 1 wk | Update test to check only action buttons (not nav spine), since SPINE is supposed to be uniform across panels. | ~10 min |
| **F-NEW-12** | Tier-2 extraction (hermes-operator) incomplete | Tech Lead | 1 wk | Decision + ADR: finish extraction (remove `gateway/operator_shell/` from hermes-agent) OR document hermes-operator as config-only repo. | ~1 day if extraction |
| **F-NEW-15** | OMS datetime bug in signal-engine (218 occurrences, upstream) | Eng Lead | 1 wk | Evaluate fork effort vs upstream patch. Create ticket with cost estimate. If fork: write patch, open PR upstream. | depends |
| **F-NEW-16** | Coverage gaps: ~23 of 53 operator_shell modules have ZERO direct tests | Eng | 1 wk | Add at minimum: regression test for F-NEW-10 fix in `sdlc.py` (env dict shape), tests for `daemons.py` (F1 critical-risk surface). | ~1 day |
