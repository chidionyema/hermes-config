# Estate Audit — 2026-06-22 01:52
_Reproducible on command (`Otto audit`). Deterministic ground-truth, nothing hidden._

## Verdict: 🔴 BROKEN

**Broken (blocks the hands-off goal):**
- Autopilot PARKED — 0 active tasks, 46 all terminal; the loop isn't advancing work.
- 27/46 tasks (58%) escalated into SILENCE — no ask-for-help handoff fires; you're never told. (R3/R4)
...
- Cron `daily-self-reflection` last_status=error (7h ago) — a scheduled loop is failing.
- Cron `proving-ground-audit` last_status=error (60m ago) — a scheduled loop is failing.
...
- ✅ `ai.hermes.gateway` — Telegram gateway (PID 5067)
- ✅ `ai.hermes.coordinator` — Autopilot coordinator (PID 42679)
...
- Tasks: **46** — 19 done · 27 escalated · 0 active · 0 awaiting-approval (done=19, escalated=27)
- Last tick: `42679|advanced=0 reaped=0`
- **Every unfinished task (27):**
...
- Verified learning ledger: 1 receipt(s); last = PASS (26h ago)
- Autonomy trend: 67% ↘︎ 41% over 31 snapshots
...
- **Learned failure-class handlers (61 — every one):**
  - `widget-fault-probe` → match `brand-new-widget-fault` · action=probe · handler=widget-probe.py
  - `widget-fault-probe` → match `brand-new-widget-fault` · action=probe · handler=widget-probe.py
  - `widget-fault-probe` → match `brand-new-widget-fault` · action=probe · handler=widget-probe.py
  - `widget-fault-probe` → match `brand-new-widget-fault` · action=probe · handler=widget-probe.py
  - `widget-fault-probe` → match `brand-new-widget-fault` · action=probe · handler=widget-probe.py
...
## 5. Self-reflection
- 5 reflections; latest `2026-06-22.md` (24m ago)

## 6. Self-healing (watchdog)
- Last run: just now
- Auto-restarts performed: gateway (25h ago)
- Wedge alerts seen: coordinator_wedged (25h ago), gateway_wedged (21h ago)
...
- **22 jobs registered (18 active / 4 paused) — every one:**
  ...
  - ▶️ **Summarize today's activity across all projects. **  `0 18 * * *`  ·  last: ok (7h ago)
      ↳ prompt: Summarize today's activity across all projects. List: which functions 
  - ▶️ **Run lux verify on all projects with specs. Repor**  `0 0 * * 0`  ·  last: ok (25h ago)
      ↳ sh: weekly-lux-verify.sh
  - ▶️ **hermes-config-auto-push**  `0 * * * *`  ·  last: ok (51m ago)
      ↳ sh: auto-push.sh
  - ▶️ **uncommitted-watch**  `every 360m`  ·  last: ok (9m ago)
      ↳ sh: uncommitted-watch.sh
  - ▶️ **daily-self-reflection**  `0 18 * * *`  ·  last: error (7h ago)
      ↳ sh: daily_reflection.py
      ⚠️ last_error: Script exited with code 1
stderr:
Reflection failed: [Errno 1] Operation not per
  - ▶️ **morning-briefing**  `0 9 * * *`  ·  last: ok (16h ago)
      ↳ skill: autonomous-ai-agents/otto-operating-model
  - ⏸ **otto-improvement-pulse**  `0 * * * *`  ·  last: ok (25h ago)
      ↳ sh: hourly_pulse.sh
      ⏸ paused: theater: blank-template hourly pulse superseded by the evidence ledger (Claude 2
  - ▶️ **idle-continuous-learning**  `every 30m`  ·  last: ok (24m ago)
      ↳ sh: idle-learning-run.sh
  - ▶️ **daily-strategist-audit**  `0 8 * * *`  ·  last: ok (17h ago)
      ↳ skill: autonomous-ai-agents/otto-operating-model
  - ▶️ **improvement-probe**  `every 15m`  ·  last: ok (4m ago)
      ↳ sh: improvement-probe.sh
  - ▶️ **health-watchdog**  `every 15m`  ·  last: ok (4m ago)
      ↳ sh: watchdog-cron.py
  - ▶️ **repo-health-check**  `every 120m`  ·  last: ok (60m ago)
      ↳ sh: repo-health-check.py
  - ▶️ **estate-inventory-audit**  `0 6 * * *`  ·  last: ok (19h ago)
      ↳ sh: estate-full-run.sh
  - ▶️ **idle-curiosity**  `every 30m`  ·  last: ok (28m ago)
      ↳ sh: idle-curiosity.py
  - ▶️ **prospector-daily-generation**  `0 * * * *`  ·  last: ok (51m ago)
      ↳ sh: prospector-run.sh
  - ▶️ **signal-engine-daemon-watchdog**  `*/5 * * * *`  ·  last: ok (1m ago)
      ↳ sh: signal-engine-daemon-watchdog.sh
  - ▶️ **proving-ground-audit**  `every 120m`  ·  last: error (60m ago)
      ↳ sh: proving-ground.py
      ⚠️ last_error: Script exited with code 1
stdout:
PROVING GROUND — Self-Integrity Audit (existen
  - ▶️ **queue-curator**  `*/5 * * * *`  ·  last: ok (1m ago)
      ↳ sh: queue-curate.sh
  - ⏸ **otto-dispatch**  `1-59/5 * * * *`  ·  last: ok (34h ago)
      ↳ sh: otto-dispatch.sh
  - ▶️ **pytest-orphan-cleanup**  `every 5m`  ·  last: ok (1m ago)
      ↳ sh: pytest-orphan-cleanup.sh
  - ⏸ **goal-of-the-moment**  `every 60m`  ·  last: ok (35h ago)
...
- **Behaviour policies (10 — every one, with the rule it enforces):**
  ...
  - `pol-20260618-002` [provisional, conf 0.3] — Use background=true for any delegate_task call that might take over 30 seconds, so the con
  - `pol-20260618-003` [provisional, conf 0.3] — When the approach is clear from the spec, dispatch fixes immediately and report after exec
  - `pol-20260618-004` [active, conf 0.8] — After EVERY user correction, run reflect-on-correction.py BEFORE the next action. Append a
  - `pol-20260618-006` [provisional, conf 0.3] — Read source code to verify API signatures before calling — never guess at method names or 
  ...
  - `pol-20260618-010` [active, conf 0.7] — If asking about whether something works/runs/is ready, execute the verification script and
  - `pol-20260618-012` [active, conf 0.8] — Test suites and builds run synchronously. Never delegate them to a subagent — use backgrou
  - `pol-auto-engineering-reliability-20260618` [provisional, conf 0.5] — Handle engineering/reliability issues proactively. If a failure in engineering/reliability
...
- 15 repos dirty (top): haworks-platform=45, .hermes=35, signalengine=21, prospector=16, the-introduction-exchange=9, modeltrainer_backup=8
