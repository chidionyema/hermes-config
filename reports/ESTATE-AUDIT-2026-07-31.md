...
- **Learned failure-class handlers (64 — every one):**
...
- **Scripts: 92 .py + 39 .sh — every one, with its purpose:**
  ...
  - `test_escalation_outbox.py` — Integration test: the LIVE coordinator.escalate() now writes the transactional
...
- **Behaviour policies (5 — every one, with the rule it enforces):**
  ...
  - `pol-auto-engineering-reliability-20260730` [provisional, conf 0.5] — Handle engineering/reliability issues proactively. If a failure in engineering/reliability
  - `pol-auto-meta-reflection-20260730` [provisional, conf 0.5] — Handle meta/reflection issues proactively. If a failure in meta/reflection occurs, create 
