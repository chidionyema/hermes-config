# Hermes Estate Inventory

**Generated:** 2026-06-24 13:32:39
---### Hermes Agent

- **Version:** 
- **Config:** /Users/chidionyema/.hermes/config.yaml (12477 bytes)
- **Profile:** default
- **Models configured:** 0
- **Skills count:** 88
- **Plugins:** 1
- **Cron jobs:** 1 (1 file = 22 jobs)
## Scripts

- **Total scripts:** 124
  - alert-resolver-probe.sh
  - alert-resolver.py
  - append-regression-trend.py
  - audit-trail.py
  - auto-push.sh
  - claude_handback_gate.py
  - closed-loop-proof.sh
  - conflict-resolver.py
  - coordinator-daemon.sh
  - coordinator.py
  - cross-project-bridge.py
  - daemon-stability-probe.sh
  - daily_reflection.py
  - dispatch-guard.py
  - dispatch_gate.py
  - dropped-ball-probe.sh
  - dropped-ball-tracker.py
  - estate-audit.py
  - estate-auto-remediation.py
  - estate-drift-detector.py
  - estate-full-run.sh
  - estate-inventory.py
  - estate-optimization-scanner.py
  - estate_alert.py
  - estate_watchdog.py
  - eval-confidence.py
  - evidence_verify.py
  - flight.py
  - gap-finding.py
  - gateway_crashloop_watch.py
  - gateway_preflight.py
  - git-pre-commit-hook.sh
  - goal-of-the-moment.sh
  - hermes_claims.py
  - hermes_fingerprint.py
  - hermes_gateway.py
  - hermes_queue.py
  - hermes_subprocess.py
  - hourly_pulse.sh
  - idle-consolidation.py
  - idle-curiosity.py
  - idle-learning-probe.sh
  - idle-learning-run.sh
  - improvement-probe.sh
  - improver-switcher.py
  - known_classes.py
  - launch-report.sh
  - memory-capacity-probe.sh
  - memory-hygiene.py
  - memory_retrieval.py
  - mentor-reflect.py
  - meta-improver.py
  - methodology-probe.sh
  - near-miss-analyzer.py
  - otto-correction-gate.py
  - otto-correction-scan-probe.sh
  - otto-correction-scan.py
  - otto-dispatch-probe.sh
  - otto-dispatch.py
  - otto-dispatch.sh
  - otto-introspect.py
  - otto-learn.py
  - otto-why.py
  - outbox.py
  - outcome-accelerator.py
  - outcome-evaluator.py
  - policy-composer.py
  - policy-enforcer.py
  - popdd-init.sh
  - post-claim-verifier.py
  - progress-snapshot.sh
  - progress.py
  - proof-probe.py
  - prospector-run.sh
  - prove_learning.py
  - prove_rsi.py
  - proving-ground-probe.py
  - proving-ground-probe.sh
  - proving-ground.py
  - publish-lux-stack.sh
  - pytest-orphan-cleanup.sh
  - queue-curate.sh
  - queue-probe.sh
  - reflect-on-correction.py
  - repo-health-check.py
  - repo-health-probe.py
  - route.py
  - rsi-autorun.sh
  - rsi-orchestrator.py
  - sandbox.py
  - self-detect.py
  - self-healer.py
  - self-regression.py
  - set-cockpit-menu.py
  - setup-embedding-model.py
  - sign-interpreters.sh
  - signal-engine-daemon-watchdog.sh
  - signal-engine-watchdog-probe.sh
  - skill-hygiene.py
  - test_async_executor.py
  - test_coordinator.py
  - test_cost.py
  - test_cutover.py
  - test_escalation_outbox.py
  - test_flight.py
  - test_outbox.py
  - test_progress_stream.py
  - test_reaper.py
  - test_resolution_disease.py
  - test_route.py
  - test_sandbox.py
  - trend-analyzer.py
  - uncommitted-watch.sh
  - warroom.py
  - warroom_eval.py
  - watchdog-cron.py
  - watchdog-probe.sh
  - watchdog-state-probe.py
  - watchdog.py
  - weekly-lux-verify.sh
  - weekly-progress-digest.py

## Skills

  **apple** (5): apple-notes, apple-reminders, findmy, imessage, macos-computer-use
  **autonomous-ai-agents** (6): agy, claude-code, codex, hermes-agent, opencode, otto-operating-model
  **creative** (16): architecture-diagram, ascii-art, ascii-video, baoyu-infographic, claude-design, comfyui, design-md, excalidraw, humanizer, manim-video, p5js, popular-web-designs, pretext, sketch, songwriting-and-ai-music, touchdesigner-mcp
  **data-science** (1): jupyter-live-kernel
  **devops** (2): kanban-orchestrator, kanban-worker
  **email** (1): himalaya
  **evaluation** (2): lm-evaluation-harness, weights-and-biases
  **github** (6): codebase-inspection, github-auth, github-code-review, github-issues, github-pr-workflow, github-repo-management
  **inference** (2): llama-cpp, vllm
  **lux-proof-driven-development** (2): popdd-on-lux, popdd-python-prototype-archived-2026-06-17
  **media** (4): gif-search, heartmula, songsee, youtube-content
  **mlops** (1): huggingface-hub
  **models** (2): audiocraft, segment-anything
  **note-taking** (1): obsidian
  **productivity** (8): airtable, google-workspace, maps, nano-pdf, notion, ocr-and-documents, powerpoint, teams-meeting-pipeline
  **research** (5): arxiv, blogwatcher, llm-wiki, polymarket, research-paper-writing
  **smart-home** (1): openhue
  **social-media** (1): xurl
  **software-development** (12): estate-management, hermes-agent-skill-authoring, hermes-self-audit, node-inspect-debugger, plan, project-health-audit, python-debugpy, requesting-code-review, simplify-code, spike, systematic-debugging, test-driven-development
  **uncategorized** (10): dogfood, dropped-ball-prevention, estate-ground-truth-probe, external-audience-writing, lux-proof-driven-development, otto-coordinator-rules-2026-06-18, popdd-inline-attestation, supervised-process-contract, task-resilience, yuanbao

## Policies

- **Total:** 10
  - pol-20260618-001: status=provisional domain=infra/process-management hits=2
  - pol-20260618-002: status=provisional domain=infra/dispatch hits=0
  - pol-20260618-003: status=provisional domain=decision-making hits=0
  - pol-20260618-004: status=active domain=meta/reflection hits=1
  - pol-20260618-006: status=provisional domain=engineering/research hits=0
  - pol-20260618-007: status=active domain=decision-making hits=1
  - pol-20260618-008: status=active domain=decision-making hits=1
  - pol-20260618-010: status=active domain=engineering/verification hits=0
  - pol-20260618-012: status=active domain=infra/dispatch hits=0
  - pol-auto-engineering-reliability-20260618: status=provisional domain=engineering/reliability hits=0

## Self-Improvement Pipeline

- **Phase 0:** Preflight (meta-improver — snapshot, off-switch, hash check)
- **Phase 0.5:** Post-correction reflection hook
- **Phase 1:** Meta-improvement analysis
- **Phase 2:** Gap-finding
- **Phase 2b:** Cross-project bridge (health → corpus)
- **Phase 2c:** Near-miss analysis
- **Phase 3:** Self-regression
- **Phase 3b:** Self-detect scan
- **Phase 4:** Policy composition + conflict resolution
- **Phase 5:** Trend analysis (cross-day)
- **Phase 6:** Consolidation
- **Phase 7:** Postflight

## Cron Jobs


  - Run health check on all projects: check for o: 0 9 * * * (last=2026-06-18T09:42, status=ok)
  - Summarize today's activity across all project: 0 18 * * * (last=2026-06-22T18:05, status=ok)
  - Run lux verify on all projects with specs. Re: 0 0 * * 0 (last=2026-06-21T00:00, status=ok)
  - hermes-config-auto-push: 0 * * * * (last=2026-06-24T05:02, status=ok)
  - uncommitted-watch: every 360m (last=2026-06-23T07:50, status=ok)
  - daily-self-reflection: 0 18 * * * (last=2026-06-22T18:00, status=ok)
  - morning-briefing: 0 9 * * * (last=2026-06-23T09:21, status=error)
  - otto-improvement-pulse: 0 * * * * (last=2026-06-21T00:00, status=ok)
  - idle-continuous-learning: every 30m (last=2026-06-24T05:04, status=ok)
  - daily-strategist-audit: 0 8 * * * (last=2026-06-23T10:20, status=ok)
  - improvement-probe: every 15m (last=2026-06-24T05:03, status=ok)
  - health-watchdog: every 15m (last=2026-06-24T05:04, status=ok)
  - repo-health-check: every 120m (last=2026-06-24T04:32, status=ok)
  - estate-inventory-audit: 0 6 * * * (last=2026-06-23T06:10, status=ok)
  - idle-curiosity: every 30m (last=2026-06-24T05:03, status=ok)
  - prospector-daily-generation: 0 * * * * (last=2026-06-24T05:00, status=ok)
  - signal-engine-daemon-watchdog: */5 * * * * (last=2026-06-24T05:10, status=ok)
  - proving-ground-audit: every 120m (last=2026-06-24T04:32, status=ok)
  - queue-curator: */5 * * * * (last=2026-06-24T05:10, status=ok)
  - otto-dispatch: 1-59/5 * * * * (last=2026-06-20T15:21, status=ok)
  - pytest-orphan-cleanup: every 5m (last=2026-06-24T05:14, status=ok)
  - goal-of-the-moment: every 60m (last=2026-06-20T14:45, status=ok)

## Memory

- **Files:** 2 markdown files
- **Tags in use:** (checked at runtime)

## ML Model

- **Model:** all-MiniLM-L6-v2 (384-dim embeddings)
- **Size:** 88743KB
- **Location:** /Users/chidionyema/.hermes/models/miniLM-onnx

## External Repos


  - signalengine: /Users/chidionyema/Documents/code/signalengine (@ )
  - lux: /Users/chidionyema/Documents/code/lux (@ )
  - prospector: /Users/chidionyema/Documents/code/prospector (@ )

## Log Directories

  - alerts/ (3 files, 1216KB)
  - audit/ (1 files, 101KB)
  - curator/ (0 files, 0KB)
  - health/ (1 files, 35KB)
  - idle-curiosity/ (314 files, 369KB)
  - improvement-pulse/ (62 files, 18KB)
  - maintenance/ (227 files, 666KB)
  - meta-improver/ (18 files, 54KB)
  - outcomes/ (1 files, 2KB)
  - reflection/ (1 files, 1KB)
  - remediation/ (1 files, 21KB)
  - retrieval/ (1 files, 40KB)
  - trends/ (195 files, 478KB)

---
## Summary

- **Scripts:** 124
- **Skills:** 88 across 20 categories
- **Policies:** 10 (5 active)
- **Cron jobs:** 22
- **Repos tracked:** 3
- **Pipeline phases:** 9 phases (preflight → postflight)
