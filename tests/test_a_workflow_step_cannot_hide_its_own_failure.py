...
THIS IS A COPY. The original lives in chidionyema/prospector as
``tests/unit/test_a_workflow_step_cannot_hide_its_own_failure.py``, next to the other half of the
same incident: ``scripts/ci_fleet_probe.py::image_staleness``, which catches the fleet running an
image older than the Dockerfile that declares openssh-client. That guard stops the failure
happening; this one stops it being invisible. Neither is sufficient alone.

It is copied rather than shared because a guard that does not run in this repository does not
guard this repository, and the failure HAPPENED here. The two repositories share no test harness
and no package.
"""
